...
def run_job(config, jobid, reltol, dt):
    ...
    simnum = 91
    md = config.parser(md, simnum)

    # Set timestep
    hour = 3600
    day = 86400
    md.timestepping.time_step = dt*hour/md.constants.yts
    out_freq = 24*1/dt
    md.settings.output_frequency = out_freq

    # Set tolerance
    md.stressbalance.restol = reltol

    print('dt:', md.timestepping.time_step*md.constants.yts)
    print('restol:', reltol)
...
def main():
    ...
    md = run_job(config, args.jobid, args.reltol, args.dt)
