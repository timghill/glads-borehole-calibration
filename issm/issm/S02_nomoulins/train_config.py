...
def parser(md, jobid):
    ...
    # Use distributed melt, no moulins
    md.hydrology.moulin_input = 0*vertices
    steady = md.basalforcings.groundedice_melting_rate
    transient = np.load('../data/melt/RACMO_mesh_runoff.npy')
    ms = transient
    ms[:-1] = ms[:-1]*md.constants.yts/1000
    ms[:-1] = ms[:-1] + steady[:,None]
    md.basalforcings.groundedice_melting_rate = ms
    return md
